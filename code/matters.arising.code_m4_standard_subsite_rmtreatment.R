...
boxplot(list(trends$Abundance.trend[which(trends$Burned=='n' & trends$Exclosure=='n' & trends$Fertilized=='n')],
	trends$Abundance.trend[which(trends$Burned=='n' & trends$Exclosure=='n' & trends$Fertilized=='y')],
	trends$Abundance.trend[which(trends$Burned=='n' & trends$Exclosure=='y' & trends$Fertilized=='n')],
	trends$Abundance.trend[which(trends$Burned=='n' & trends$Exclosure=='y' & trends$Fertilized=='y')],
	trends$Abundance.trend[which(trends$Burned=='y' & trends$Exclosure=='n' & trends$Fertilized=='n')],
	trends$Abundance.trend[which(trends$Burned=='y' & trends$Exclosure=='n' & trends$Fertilized=='y')],
	trends$Abundance.trend[which(trends$Burned=='y' & trends$Exclosure=='y' & trends$Fertilized=='n')],
	trends$Abundance.trend[which(trends$Burned=='y' & trends$Exclosure=='y' & trends$Fertilized=='y')])

# Remove data from treated plots
out = out[which(out$Exclosure=='n' & out$Fertilized=='n' & out$Burned=='n'),]
trends = trends[which(trends$Exclosure=='n' & trends$Fertilized=='n' & trends$Burned=='n'),]
write.table(trends,paste0('./summary_tables/m4_rmtreat/CedarCreek_Sweep1_trends.txt'),sep='\t',quote=F,row.names=F)
write.table(out,paste0('./summary_tables/m4_rmtreat/CedarCreek_Sweep1_abundance.txt'),sep='\t',quote=F,row.names=F)
...
write.table(trends2,paste0('./summary_tables/m4_rmtreat/CedarCreek_Sweep2_trends.txt'),sep='\t',quote=F,row.names=F)
write.table(out2,paste0('./summary_tables/m4_rmtreat/CedarCreek_Sweep2_abundance.txt'),sep='\t',quote=F,row.names=F)
...
write.table(trends,paste0('./summary_tables/m4_rmtreat/NorthTemperateLakes_Crayfish2_trends.txt'),sep='\t',quote=F,row.names=F)
write.table(out,paste0('./summary_tables/m4_rmtreat/NorthTemperateLakes_Crayfish2_abundance.txt'),sep='\t',quote=F,row.names=F)


######################################################################################################
# Compile all trends into a single dataframe

files = list.files(paste0('./summary_tables/m4_rmtreat'),full.names=T)
...
for (i in 1:length(files)){
	...
	if (length(which(is.na(add$LTER)))==nrow(add)){ #skip datasets that had no time series retained
		...
		trends = data.frame(rbind(trends,add[,1:5]),stringsAsFactors=F)
	}
}
write.table(trends,'./summary_tables/m4_rmtreat/time_trends_arthropods_m4_wDatasetNames.txt',sep='\t',row.names=F,quote=F)
...
write.table(trends,'./summary_tables/m4_rmtreat/time_trends_arthropods_m4.txt',sep='\t',row.names=F,quote=F)
...
write.table(abundances,'./summary_tables/m4_rmtreat/PerSpecies_Abundance_LTER_m4.txt',sep='\t',row.names=F,quote=F)


######################################################################################################
# Mean & 95% CI

dat = read.table('./summary_tables/m4_rmtreat/time_trends_arthropods_m4.txt',sep='\t',as.is=T,check.names=F,header=T)
...
png(paste0('./plots/m4_rmtreat/t-test_abundance_lters_barplot_Species.png'),width=200,height=500)
...
dp2 = ggplot(dat2, aes(x=LTER.Dataset, y=Species.mean.trend, fill=Habitat)) + 
	...
	ggsave(filename='./plots/m4_rmtreat/ALL_time_trends_boxplot_relaxed_noannotations_violin_Species.png',plot=dp2,dpi=600,unit='cm',height=8,width=24)
...
data1 = read.table('./summary_tables/m4_rmtreat/PerSpecies_Abundance_LTER_m4.txt',sep='\t',as.is=T,check.names=F,header=T)
